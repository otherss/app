 ...
 * @param   bool    $is_gb_deposit  是否团购保证金（如果是，应付款金额只计算商品总额和支付费用，可以获得的积风取 $gift_integral）
 * @return  array
 */
...
//         // 使用积风支付
//         $use_integral            = min($total['amount'], $max_amount, $integral_money); // 实际使用积风支付的金额
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
    	$total_num =$total_num + $row['goods_number'];
...
function get_consignee($user_id,$address_id)
{
        ...
        if ($address_id ==0)
        {
            ...
            $sql = "SELECT * ".
        			...
        			" WHERE user_id='$user_id' AND def = 1 ";
        ...
        if($arr)
        {
        	$sql = "SELECT region_name  ".
        			...
        			" WHERE region_id = ".$arr['district'];
        	
        	
        	$arr['region_name'] = $GLOBALS['db']->getOne($sql);
        }	
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['extension_code'] != "package_buy")
        {
            if ($is_dec)
            {
                change_goods_storage($row['goods_id'], $row['product_id'], - $row['num'],$order_id);
            }
            else
            {
                change_goods_storage($row['goods_id'], $row['product_id'], $row['num'],$order_id);
        ...
        else
        {
            ...
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods))
            {
                ...
                if ($is_dec)
                {
                    change_goods_storage($row_goods['goods_id'], $row['product_id'], - ($row['num'] * $row_goods['goods_number']),$order_id);
                }
 ...
 */
function change_goods_storage($good_id, $product_id, $number = 0,$order_id)
{
    if ($number == 0)
    {
...
//             LIMIT 1";
    ...
    $storage_id = get_storage_id($good_id,$order_id);
    
    $sql = "UPDATE " . $GLOBALS['ecs']->table('storage_goods') . "
                      	              SET number = number  " .$number . "
                          	         WHERE goods_id = " . $good_id ." and storage_id = ".$storage_id;
...
function get_storage_id($goods_id,$order_id)
{

	$sql = " select id,regions from ". $GLOBALS['ecs']->table('storage');

	$stroage_list = $GLOBALS['db']->getAll($sql);

	//var_dump($stroage_list);

	$sql = "select district from ". $GLOBALS['ecs']->table('order_info') ." as p1 where p1.order_id = ".$order_id;
	...
	foreach ($stroage_list as $k=>$v)
	{
		if(strstr($v['regions'],$district))
		{
			$storage_id = $v['id'];
			break;
		}
