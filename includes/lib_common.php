 ...
 * @return      array
 */
function get_regions($type = 0, $parent = 0,$is_storage=0)
{
    ...
    if($is_storage ==1)
    {
    	$sql = " select regions from " . $GLOBALS['ecs']->table('storage');
    	
    	$storage = $GLOBALS['db']->GetAll($sql);
    	
    	foreach ($storage as $k => $v)
    	{
    		foreach ($regions as $kk => $vv)
    		{
    			if(strstr($v['regions'],$vv['region_id']))
    			{
    				unset($regions[$kk]);
    			}
    		}
...
function get_regions2($type = 0, $parent = 0)
{
	$sql = 'SELECT regions FROM ' . $GLOBALS['ecs']->table('storage');
...
function get_children_by_list($cat_list = '')
{
	
	$sql = 'select cat_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id in  (".$cat_list.")";
	...
	return 'g.cat_id ' . db_create_in(array_unique(array_merge($cat, array_keys($temp))));
}
/**
 * 获得指定文章分类下所有底层分类的ID
 *
...
{
    return db_create_in(array_unique(array_merge(array($cat), array_keys(article_cat_list($cat, 0, false)))), 'c.cat_id');
}
 ...
 * @param   int     $rank_points    等级积风变动
 * @param   int     $pay_points     消费积风变动
 ...
 * @return      array
 */
function get_regions_options($type = 0, $parent = 0)
{
	$sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['ecs']->table('region') .
	" WHERE region_type = '$type' AND parent_id = '$parent'";
	...
	foreach ($res as $k=>$v)
	{
		$str .="<option id='".$v['region_id']."'>".$v['region_name']."</option>";
	}
