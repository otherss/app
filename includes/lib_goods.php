...
function get_categories_top($parent_id)
{
...
//     }
    ...
    */
        ...
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,dirPath ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $k => $row)
        {
                    ...
                    $res[$k]['child'] = get_child_tree($row['cat_id']);
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show,dirPath ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        $three_arr = $GLOBALS['db']->getAll($child_sql);
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $page,$size)
{
...
//     $price_where .= ($max > 0) ? " AND g.shop_price <= $max " : '';
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price , g.promote_price, ' .
                ...
                'promote_start_date, promote_end_date,  g.goods_thumb, goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    switch ($type)
    {
        ...
        case 'promote':
            $time = gmtime();
            $sql .= " AND (( is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ) or is_best =1) ";
            break;
    }
...
function get_goods_info($goods_id)
{
    ...
    $time = gmtime();
    $sql = 'SELECT g.goods_brief,g.goods_id, g.goods_desc , g.goods_name, g.goods_thumb,g.goods_number,g.original_img, g.goods_img, g.shop_price ,g.market_price ' .
...
//     $arr['lnk'] = array();     // 关联的属性
    ...
    foreach ($res AS $row)
    {
        ...
        $temp ['name']  = $row['attr_name'];
        $temp ['value'] = $row['attr_value'];
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$keyword='',$cat_id)
{
	
	if($cat_id>0)
	{
		
		$where = "g.is_on_sale = 1  AND g.goods_name like '%".$keyword."%' AND ".
				"g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
	}
	else
	{
...
// 	echo $sql;
	...
	$temp['market_price']     = price_format($row['market_price']);
	$temp['shop_price']       = price_format($row['org_price']);

	$temp['promote_price']    = ($promote_price > 0) ? price_format($promote_price) : '';
	//         $temp['goods_thumb']      = get_image_path($row['goods_id'], $row['goods_thumb'], true);
	$temp['goods_img']        = get_image_path($row['goods_id'], $row['goods_img']);
	$temp['goods_thumb']        = get_image_path($row['goods_id'], $row['goods_img']);
...
// 		echo $price;
		...
		if(strstr($price, "range1"))
		{
			
			$ext .=" OR g.shop_price <=100 ";
		}
		
		if(strstr($price, "range2"))
		{
			$ext .=" OR ( g.shop_price >100  AND g.shop_price <=200 ) ";
		}
		
		if(strstr($price, "range3"))
		{
			$ext .=" OR ( g.shop_price >200  AND g.shop_price <=400 ) ";
		}
		
		if(strstr($price, "range4"))
		{
			$ext .=" OR ( g.shop_price >400  AND g.shop_price <=600 ) ";
		}
		
		if(strstr($price, "range5"))
		{
			$ext .=" OR ( g.shop_price >600  AND g.shop_price <=1000 ) ";
		}
		
		if(strstr($price, "range6"))
		{
			$ext .=" OR ( g.shop_price >1000 ) ";
		}
