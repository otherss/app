...
function insert_cart_info()
{
    ...
    $GLOBALS['smarty']->assign('str','购物车 '. $number .' 件');
...
function insert_get_praise_nums($arr)
{
	
	$sql = 'SELECT praise_nums FROM '.$GLOBALS['ecs']->table('share'). ' where id = '.$arr['goods_id'];
	
	return $GLOBALS['db']->getOne($sql);
}


function insert_get_comment_nums($arr)
{

	$sql = 'SELECT comment_nums FROM '.$GLOBALS['ecs']->table('share'). ' where id = '.$arr['goods_id'];
...
function insert_share_comments($arr)
{
	$need_cache = $GLOBALS['smarty']->caching;
	$need_compile = $GLOBALS['smarty']->force_compile;
	...
	$GLOBALS['smarty']->assign('comment_type', $arr['type']);
	$GLOBALS['smarty']->assign('id',           $arr['id']);
	$cmt = assign_share_comment($arr['id'],          $arr['type']);
	$GLOBALS['smarty']->assign('comments',     $cmt['comments']);
	$GLOBALS['smarty']->assign('pager',        $cmt['pager']);

	
	$val = $GLOBALS['smarty']->fetch('library/share_comments_list.lbi');
...
function insert_get_pay_points()
{
