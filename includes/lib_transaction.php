...
//             $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['email_invalid'], $profile['email']));
            ...
            return sprintf($GLOBALS['_LANG']['email_invalid'], $profile['email']);
        }
    ...
    if (!$GLOBALS['user']->edit_user($cfg))
    {
        ...
        return sprintf($GLOBALS['_LANG']['email_exist'], $profile['email']);
    }
...
function get_consignee_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id'   order by  address_id desc LIMIT 20 ";
 ...
 * @return  array
 */
function get_consignee_one($user_id,$address_id)
{
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
 ...
 * @return  array
 */
function get_default_consignee($user_id,$address_id)
{
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
	" WHERE user_id = '$user_id' and address_id = '$address_id' LIMIT 1";
	...
	{
		$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
		" WHERE user_id = '$user_id'  order by def desc ";
 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0,$from,$where="" )
{
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT is_comment,order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($row['shipping_status']==2 && $row['is_comment'] ==1)
        {
        	$row['status'] = "已完成";
        }
        ...
        else if($row['pay_status']==2 && $row['shipping_status']==1)
        {
        	$row['status'] = "待收货";
        }
        else if($row['pay_status']==2 && $row['shipping_status']!=1)
...
//        $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']] . ',' . $GLOBALS['_LANG']['ps'][$row['pay_status']] . ',' . $GLOBALS['_LANG']['ss'][$row['shipping_status']];
        ...
        $goods = order_goods2($row['order_id']);
 ...
 * @return  boolean
 */
function drop_collection($id)
{
	
		$sql = "DELETE FROM " .$GLOBALS['ecs']->table('collect_goods') . " WHERE rec_id = '$id'";
		$res = $GLOBALS['db']->query($sql);
...
function default_consignee($id)
{
	...
	else
	{
		...
		$sql = "UPDATE  " .$GLOBALS['ecs']->table('user_address') . "  SET  def =0  WHERE user_id = '$uid'";
		$res = $GLOBALS['db']->query($sql);
		
		$sql = "UPDATE  " .$GLOBALS['ecs']->table('user_address') . "  SET  def = 1  WHERE address_id = '$id'";
		$res = $GLOBALS['db']->query($sql);
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    $order = formated_consignee($order);
//     var_dump($order);
    //检查订单是否属于该用户
    ...
    $order['shipping_status'] = ($order['shipping_status'] == SS_SHIPPED_ING) ? SS_PREPARING : $order['shipping_status'];
    ...
    else if($order['order_status']==2)
    {
 	...
 	else if($order['pay_status']==2 && $order['shipping_status']==1)
    {
        $order['status'] = "待收货";
    }
    else if($order['pay_status']==2 && $order['shipping_status']!=1)
...
function formated_consignee($consignee)
{
	$range =array();


	$range[]=$consignee['province'];
	$range[]=$consignee['city'];
	$range[]=$consignee['district'];
	...
	{
		if($consignee['province'] == $v1['region_id'])
		{
			$consignee['p_name'] = $v1['region_name'];
		}

		if($consignee['city'] == $v1['region_id'] && $v1['region_name']!=$consignee['p_name'])
		{
			$consignee['c_name'] = $v1['region_name'];
		}

		if($consignee['district'] == $v1['region_id'])
		{
			$consignee['d_name'] = $v1['region_name'];
		}
