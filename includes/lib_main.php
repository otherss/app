 ...
 * @return  array
 */
function assign_share_comment($id, $type, $page = 1)
{
	/* 取得评论列表 */
	$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('share_comment').
			" WHERE sid = '$id' AND type = '$type'  AND pid = 0  and status = 1" );
	$size  = !empty($GLOBALS['_CFG']['comments_number']) ? $GLOBALS['_CFG']['comments_number'] : 5;

	$page_count = ($count > 0) ? intval(ceil($count / $size)) : 1;

	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('share_comment') .
	" WHERE sid = '$id' AND type = '$type'  AND pid = 0 and status = 1".
	' ORDER BY id DESC';
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$arr[$row['id']]['uname'] = $row['uname'];
		$arr[$row['id']]['content']  = str_replace('\r\n', '<br />', htmlspecialchars($row['content']));
		$arr[$row['id']]['content']  = nl2br(str_replace('\n', '<br />', $arr[$row['id']]['content']));
		
		$arr[$row['id']]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
	}
	/* 取得已有回复的评论 */
	if ($ids)
	{
		...
		while ($row = $GLOBALS['db']->fetch_array($res))
		{
			$arr[$row['pid']]['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
			$arr[$row['pid']]['re_add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
		
			$arr[$row['pid']]['re_uame'] = $row['uname'];
		}
	}
	/* 分页样式 */
	//$pager['styleid'] = isset($GLOBALS['_CFG']['page_style'])? intval($GLOBALS['_CFG']['page_style']) : 0;
	...
	$pager['record_count'] = $count;
	$pager['page_count']   = $page_count;
	$pager['page_first']   = "javascript:gotoPage2(1,$id,$type)";
	$pager['page_prev']    = $page > 1 ? "javascript:gotoPage2(" .($page-1). ",$id,$type)" : 'javascript:;';
	$pager['page_next']    = $page < $page_count ? 'javascript:gotoPage2(' .($page + 1) . ",$id,$type)" : 'javascript:;';
	$pager['page_last']    = $page < $page_count ? 'javascript:gotoPage2(' .$page_count. ",$id,$type)"  : 'javascript:;';
...
function user_info($user_id)
{
    ...
    $sql = " select count(*) as num from ". $GLOBALS['ecs']->table('order_info') ." where user_id = ".$user_id." and shipping_status = 2  AND is_comment =0 ";
    $comment_nums = $GLOBALS['db']->getOne($sql);
    
    $sql = " select count(*) as num from ". $GLOBALS['ecs']->table('order_info') ." where user_id = ".$user_id." and pay_status != 2 and order_status!=2 ";
    $pay_nums = $GLOBALS['db']->getOne($sql);
    
    $sql = " select count(*) as num from ". $GLOBALS['ecs']->table('order_info') ." where user_id = ".$user_id." and  (shipping_status =0 or shipping_status =3) and pay_status=2 ";
    $send_nums = $GLOBALS['db']->getOne($sql);
    
    $sql = " select count(*) as num from ". $GLOBALS['ecs']->table('order_info') ." where user_id = ".$user_id." and  shipping_status =1  and pay_status=2 ";
    $confirm_nums = $GLOBALS['db']->getOne($sql);
    
    $user['comment_nums'] = $comment_nums;
    $user['pay_nums'] = $pay_nums;
    $user['send_nums'] = $send_nums;
    $user['confirm_nums'] = $confirm_nums;
