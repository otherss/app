...
//         }
    ...
    function edit_user($cfg)
    {
        ...
        if ((!empty($cfg['realname'])))
        {
        	$values[] =  "realname='" . $cfg['realname'] . "'";
        }
     ...
     * @return  int
     */
    function check_user_app($user_id = 0, $user_key = "")
    {

       
            $sql = "SELECT user_key,user_name  FROM " . $this->table("users").
                   " WHERE  user_id = ".$user_id;

           $res = $this->db->getRow($sql);
           
           if($user_key!="" && $user_key == $res['user_key'])
           {
