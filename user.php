...
/* 显示页面的action列表 */
$ui_arr = array('set_default_address','register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','share_comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','share','');
...
if ($action == 'default')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('to_user').
    ...
    $pager = get_pager('user.php', $act, $record_count, $page, 6);
    
    $smarty->assign('message_list', get_to_user_list($user_id,  $pager['size'], $pager['start']));
    $smarty->assign('pager',        $pager);
    
//     var_dump(get_to_user_list($user_id,  $pager['size'], $pager['start']));
//     $smarty->assign('order_info',   $order_info);
//     $smarty->display('user_clips.dwt');
    
    
    $smarty->assign('info',        get_user_default($user_id));
    $smarty->assign('cur_rank',        $_SESSION['user_rank']);
...
//     $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id'");
    ...
    $record_count = get_order_recordcount($user_id,$flag);
    
    $pager  = get_pager('user.php', array('act' => $action,'flag' =>$flag), $record_count, $page,6);

    $orders = get_my_user_orders($user_id, $pager['size'], $pager['start'],$flag);
...
elseif ($action == 'address_list')
{
	...
	$add_id = $_REQUEST ['address_id'];
	
	$smarty->assign( 'country_list', get_regions() );
	
	if($add_id)
	{
		$sql = "SELECT * FROM " . $ecs->table( 'user_address' ) . " WHERE address_id = " . $add_id;
		
		$edit_address = $db->getAll( $sql );
		
		$smarty->assign( 'edit_address', $edit_address );
		
		$smarty->assign( 'ed_province', $edit_address [0] ['province'] );
		$smarty->assign( 'ed_city', $edit_address [0] ['city'] );
		$smarty->assign( 'ed_country', $edit_address [0] ['country'] );
		$smarty->assign( 'ed_district', $edit_address [0] ['district'] );
		
		
		$province_list = get_regions( 1, $edit_address [0] ['country'] );
		$city_list = get_regions( 2, $edit_address [0] ['province'] );
		$district_list = get_regions( 3, $edit_address [0] ['city'] );
	...
	if (isset( $_REQUEST ['flag'] ))
	{
		...
		if ($_REQUEST ['flag'] == 'del' && isset( $_REQUEST ['address_id'] ))
		{
				
			$add_id = $_REQUEST ['address_id'];
			$sql = "delete from " . $ecs->table( 'user_address' ) . " where address_id=" . $add_id;
		...
		else
		{
			...
			$str = $smarty->fetch("library/address.lbi");
			echo $str;
			exit;
		}
	}
...
// 	exit;
	...
	$format_consignee_list = formated_consignee_list($consignee_list);
	
	$smarty->assign( 'consignee_list', $format_consignee_list );
	
	$smarty->assign( 'consignee_count', count($format_consignee_list) );
...
elseif ($action == 'share_comment_list')
{
    include_once(ROOT_PATH . 'includes/lib_clips.php');
    ...
    /* 获取用户留言的数量 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('share_comment').
           " WHERE pid = 0 AND ower = '".$user_id."' and status = 1 and type = 0";
    $record_count = $db->getOne($sql);
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page, 5);

    $smarty->assign('share_comment_list', get_user_comment_list($user_id, $pager['size'], $pager['start']));
    $smarty->assign('pager',        $pager);
    $smarty->display('user_clips.dwt');
}
...
elseif ($action == 'account_detail')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('account_log').
           ...
           " AND $account_type <> 0  AND change_type not in (100)";
  ...
  $surplus_amount = get_user_credit($user_id);
...

    {
        ...
        {
        	$row['short_change_desc'] = '管理员调节,'.sub_str($row['change_desc'], 60);
        }
        else 
        {
    ...
    $surplus_sign = get_sign_credit($user_id);
    
    if (empty($surplus_sign)){
    
    	$surplus_sign = 0;
    
    }
    
    $smarty->assign('surplus_sign', $surplus_sign);
    //模板赋值
    $smarty->assign('rest', $surplus_amount - $surplus_sign);
...
/* 删除晒单回复 */
elseif ($action == 'del_share_reply')
{
    $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    if ($id > 0)
    {
        $sql = "DELETE FROM " .$ecs->table('share_comment'). " WHERE pid != 0 and id = '$id' AND ower = '$user_id'";
        $db->query($sql);
    }
    ecs_header("Location: user.php?act=share_comment_list\n");
    exit;
}

/* 添加晒单回复 */
elseif ($action == 'share_reply')
{
	include_once(ROOT_PATH .'includes/cls_json.php');
	include_once(ROOT_PATH .'includes/lib_transaction.php');
	$json = new JSON();
    ...
    if ($id > 0)
    {
    	$sql = 'select count(*) from '.$ecs->table('share_comment').' where pid = '.$id ;
    	
    	if($db->getOne($sql)>0)
    	{
    		$result['success'] = 0;
    		$result['content'] = '该评论已回复，不能重复操作';
    	}
    	else 
    	{
    		
	    	$_time = gmtime();
	        $sql = "insert into " .$ecs->table('share_comment'). " (pid,content,type,add_time) values (".$id." ,'".$content."',1,".$_time.")";
	        if(!$db->query($sql))
	        {
	        	$result['success'] = 0;
	        	$result['content'] = '操作失败';
...
// 	        echo $sql;
	        	$result['reply_id'] = $db->insert_id();
	        	$result['time'] = date('Y-m-d H:i:s', $_time);
    	}
...
elseif ($action == 'act_transform_points')
{
    ...
    switch ($rule_key)
    {
        case TO_P :
            ...
            $user->set_points($row['user_name'], array($bbs_key=>0 - $num)); //调整论坛积风
        ...
        case FROM_P :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0, 0-$num, $_LANG['transform_points'], ACT_OTHER); //调整商城积风
            $user->set_points($row['user_name'], array($bbs_key=>$result_points)); //调整论坛积风
        ...
        case FROM_R :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0-$num, 0, $_LANG['transform_points'], ACT_OTHER); //调整商城积风
            $user->set_points($row['user_name'], array($bbs_key=>$result_points)); //调整论坛积风
...
else if($action == 'share')
{
	...
	$is_share = get_order_info($order_id);
	$goods = get_order_goods_info($order_id);
	
	if($is_share>0)
	{
		die("invalid");
	}
	if(!can_share($order_id) )
	{
		die("invalid");
	}
	// 	var_dump($goods);
	...
	{
		$upload_size_limit = ($upload_size_limit/1024).'m';
	}
	else
	{
		$upload_size_limit = $upload_size_limit.'k';
	}
	
	$smarty->assign('upload_size_limit', $upload_size_limit);
	
	$smarty->assign('refund_goods_list', $goods);
	$smarty->assign('order_id', $order_id);
	$smarty->display("user_transaction.dwt");
}

else if('act_share' == $action)
{
	$order_id = $_POST['order_id'];
	
	$refund = $_POST;
	unset($refund['order_id']);
	$refund['pic1'] = (isset($_FILES['pic1']['error']) && $_FILES['pic1']['error'] == 0) || (!isset($_FILES['pic1']['error']) && isset($_FILES['pic1']['tmp_name']) && $_FILES['pic1']['tmp_name'] != 'none')
	? $_FILES['pic1'] : array();
	$refund['pic2'] = (isset($_FILES['pic2']['error']) && $_FILES['pic2']['error'] == 0) || (!isset($_FILES['pic2']['error']) && isset($_FILES['pic2']['tmp_name']) && $_FILES['pic2']['tmp_name'] != 'none')
	? $_FILES['pic2'] : array();
	$refund['pic3'] = (isset($_FILES['pic3']['error']) && $_FILES['pic3']['error'] == 0) || (!isset($_FILES['pic3']['error']) && isset($_FILES['pic3']['tmp_name']) && $_FILES['pic3']['tmp_name'] != 'none')
	? $_FILES['pic3'] : array();
	if(refund_apply_order_goods($refund, $order_id))
	{
		show_message("成功晒单", "订单列表", "user.php?act=order_list&flag=done");
	}
	else
	{
		$GLOBALS['err']->show("返回重新晒单", 'user.php?act=share&order_id='.$order_id);
	}

}


function refund_apply_order_goods($refund, $order_id)
{
	if($order_id<=0 || $GLOBALS['db']->getOne("select is_share from ".$GLOBALS['ecs']->table("order_info")." where order_id='$order_id'")>0)
	{
		die("invalid");
	}
	...
	$pic = false;
	if((!empty($refund['msg1']))&&(!empty($refund['pic1'])))
	{
		$pic = true;
	}
	if((!empty($refund['msg2']))&&(!empty($refund['pic2'])))
	{
		$pic = true;
	}
	if((!empty($refund['msg3']))&&(!empty($refund['pic3'])))
	{
		$pic = true;
	}
	if(!$pic)
	{
		...
		$GLOBALS['err']->add("至少上传一图一文！");
		return false;
	}
	
	if (empty($refund['gid']))
	{
		$GLOBALS['err']->add("必须选择晒单产品！");
		return false;
	}
	if (empty($refund['title']))
	{
		$GLOBALS['err']->add("必须填写晒单主题！");
		return false;
	}
	if (empty($refund['msg']))
	{
		$GLOBALS['err']->add("必须填写晒单简介！");
		return false;
	}
	

	$refund['pic1'] = refund_apply_order_goods_upload_ex($refund, 'pic1', $upload_size_limit);
	$refund['pic2'] = refund_apply_order_goods_upload_ex($refund, 'pic2', $upload_size_limit);
	$refund['pic3'] = refund_apply_order_goods_upload_ex($refund, 'pic3', $upload_size_limit);
	if($refund['pic1'] < 0 || $refund['pic2'] < 0 || $refund['pic3'] < 0)
	{
		return false;
	}
	$refund['status'] = 1;
	$refund['integrate'] = 100;
	$refund['add_time'] = gmtime();
	$refund['order_id'] = $order_id;
	$user_id = $_SESSION['user_id'];
	$refund['uid'] = $user_id;
	$refund['user_name'] = get_user_name($user_id);
	$data['is_share'] = 1;
// 	$data['uid'] = $user_id;
// 	$data['gid'] = $refund['good_id'];

	$back2 = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('share'), $refund, 'INSERT');
	
	$data['share_id'] =$GLOBALS['db']->insert_id();
	$back1 = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $data, 'UPDATE', "order_id = '" . $order_id ."'");
...
{
	$sql = "select og.goods_name,og.goods_price,og.goods_price*og.goods_number as subtotal,og.goods_number,og.goods_id,g.goods_thumb,og.rec_id,o.order_id from ".$GLOBALS['ecs']->table("order_goods")." as og,".$GLOBALS['ecs']->table("order_info")." as o,".$GLOBALS['ecs']->table("goods")." as g where og.goods_id=g.goods_id and og.order_id=o.order_id and o.order_id='$order_id'";
	$row = $GLOBALS['db']->getAll($sql);
	foreach ($row as $k => $v)
	{

		$row[$k]['url'] = build_uri('goods', array('gid'=>$row[$k]['goods_id']) );
	}
	return $row;
}
...
{
	$sql = "select is_share from ".$GLOBALS['ecs']->table("order_info")." where order_id='$order_id'";
 ...
 * @return  array
 */
function get_user_comment_list($user_id, $page_size, $start)
{
	$sql = "SELECT c.*,  r.id as reply_id , r.content AS reply_content, r.add_time AS reply_time ".
			" FROM " . $GLOBALS['ecs']->table('share_comment') . " AS c ".
			" LEFT JOIN " . $GLOBALS['ecs']->table('share_comment') . " AS r ".
			" ON r.pid = c.id AND r.pid > 0 ".
			" WHERE c.ower='$user_id' and c.type = 0  and c.status = 1 order by c.add_time desc";
...
function get_sign_credit($user_id)
{

	$sql = "SELECT sum(pay_points ) FROM " .$GLOBALS['ecs']->table('account_log').
	" WHERE user_id = '$user_id'  and change_type = 100";

	return $GLOBALS['db']->getOne($sql);
}

function get_order_recordcount($user_id , $flag='')
{
	...
	if($flag=='done')
	{
		$where = "where order_status=2 or shipping_status =2 and user_id = '$user_id'";
	}
	else if($flag=='un_pay')
	{
		$where = "where order_status in (0,1)  and pay_status = 0 and  user_id = '$user_id'";
	}
	else if ($flag == 'pay')
	{
		$where = "where pay_status=2 and shipping_status != 2 and  user_id = '$user_id'";
	}
...
/* $flag=doing 获取正在进行中的订单，默认此操作 $flag=done 获取已完成的订单 $flag= return_goods //获取退货订单 */
function get_my_user_orders($user_id, $num = 20, $start = 0, $flag)
{
	/* 取得订单列表 */
	$arr = array ();
	...
	*/
	...
	$sql = "SELECT order_id,share_id, order_sn, consignee,order_status, is_share, shipping_status, pay_status, add_time,pay_name, " . "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee " . " FROM " . $GLOBALS ['ecs']->table( 'order_info' ) . "  $where  ORDER BY add_time DESC";
	
	$res = $GLOBALS ['db']->SelectLimit( $sql, $num, $start );

	while ( $row = $GLOBALS ['db']->fetchRow( $res ) )
	{
		$sql2 = "select g.goods_thumb,g.goods_id,g.goods_name from " . $GLOBALS ['ecs']->table( "goods" ) . " as g ," . $GLOBALS ['ecs']->table( "order_goods" ) . " as og, " . $GLOBALS ['ecs']->table( "order_info" ) . " as oi where oi.order_id=og.order_id and og.goods_id=g.goods_id and oi.user_id = '$user_id' and oi.order_id=" . $row ['order_id'];

		$order_con = $GLOBALS ['db']->getAll( $sql2 );

		unset( $img );

		foreach ( $order_con as $val )
		{
				
			$img .= "<a href='../goods.php?id=" . $val ['goods_id'] . "'><img src='../" . $val ['goods_thumb'] . "' /></a>";
		}
		...
		if ($row ['shipping_status'] == SS_SHIPPED)
		{
			...
			@$row ['handler'] = "<div  class='btn_pay'><a href=\"user.php?act=affirm_received&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_received'] . "')) return false;\">" . $GLOBALS ['_LANG'] ['received'].'</div><br>';
		} else if ($row ['pay_status'] == PS_UNPAYED && $row ['order_status'] != OS_RETURNED && $row ['order_status'] != OS_CANCELED)
		{
			$row ['handler'] = "<div  class='btn_pay'><a href=\"user.php?act=order_detail&order_id=" . $row ['order_id'] . '">立即支付</a></div><br>';
// 			@$row ['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_cancel'] . "')) return false;\">取消</a>";
		}
		else if (  $row ['pay_status'] ==PS_PAYED &&$row ['shipping_status'] == SS_RECEIVED && $row ['is_share'] == 0)
		{
			$row ['handler'] = "<div  class='btn_pay'><a href=\"user.php?act=share&order_id=" . $row ['order_id'] . '">我要晒单</a></div><br>';
			// 			@$row ['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_cancel'] . "')) return false;\">取消</a>";
		}
		else if (  $row ['pay_status'] ==PS_PAYED &&$row ['shipping_status'] == SS_RECEIVED && $row ['is_share'] == 1)
		{
			$row ['handler'] = "<a class='f4'  href='javascript:void(0)'>晒单审核中</a>";
			// 			@$row ['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_cancel'] . "')) return false;\">取消</a>";
		}
		else if (  $row ['pay_status'] ==PS_PAYED &&$row ['shipping_status'] == SS_RECEIVED && $row ['is_share'] == 2)
		{
			$row ['handler'] = "<div  class='btn_pay'><a class='f4'  href='share_info.php?id=".$row ['share_id']."'>查看晒单</a></div><br>";
			// 			@$row ['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_cancel'] . "')) return false;\">取消</a>";
		}
		else if (  $row ['pay_status'] ==PS_PAYED && $row ['shipping_status'] == SS_RECEIVED && $row ['is_share'] == 3)
		{
			$row ['handler'] = "<a href='javascript:void(0)'" . $row ['order_id'] . '">晒单未通过</a>';
			// 			@$row ['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" . $row ['order_id'] . "\" onclick=\"if (!confirm('" . $GLOBALS ['_LANG'] ['confirm_cancel'] . "')) return false;\">取消</a>";
		}

		@$row ['handler'] .= " <a href=\"user.php?act=order_detail&order_id=" . $row ['order_id'] . '">查看</a>';
		...
		{
			$row ['order_status'] = '等待付款';
		}  else if ($row ['shipping_status'] != SS_SHIPPED && $row ['shipping_status'] != SS_RECEIVED && $row ['order_status'] != OS_RETURNED)
		{
			$row ['order_status'] = '等待发货';
		} else if ($row ['shipping_status'] == SS_SHIPPED)
		{
			$row ['order_status'] = '已发货';
		} else if ($row ['shipping_status'] == SS_RECEIVED)
		{
			$row ['order_status'] = '已收货';
		} else if ($row ['order_status'] == OS_RETURNED)
		{
			$row ['order_status'] = '退货';
		} 

		$arr [] = array (
				'order_id' => $row ['order_id'],'order_sn' => $row ['order_sn'],'order_time' => local_date( $GLOBALS ['_CFG'] ['time_format'], $row ['add_time'] ),'order_status' => $row ['order_status'],'total_fee' => price_format( $row ['total_fee'], false ),'handler' => $row ['handler'],
				'pay_name' => $row ['pay_name'],'consignee' => $row ['consignee'],'goods_thumb' => $img
		)
		;
	}
...
function formated_consignee_list($consignee_list)
{
	$range =array();
	...
	$range = db_create_in($range,"region_id");
	$sql =" select region_id ,region_name from ".$GLOBALS ['ecs']->table( "region" ) ." where ".$range;
	
	$region_names = $GLOBALS ['db']->getAll($sql);
	
	foreach ($consignee_list as $k => $v)
		{
			foreach ($region_names as $k1 => $v1)
			{
				if($v['province'] == $v1['region_id'])
				{
					$consignee_list[$k]['p_name'] = $v1['region_name'];
				}
				
				if($v['city'] == $v1['region_id'] && $v1['region_name']!=$consignee_list[$k]['p_name'])
				{
					$consignee_list[$k]['c_name'] = $v1['region_name'];
				}
				
				if($v['district'] == $v1['region_id'] )
				{
					$consignee_list[$k]['d_name'] = $v1['region_name'];
				}
			}
		}
...
function get_to_user_list($user_id, $num, $start)
{
	/* 获取留言数据 */
	$msg = array();
	$sql = "SELECT t.*,a.user_name as admin_name FROM " .$GLOBALS['ecs']->table('to_user') ."as t ,  ".$GLOBALS['ecs']->table('admin_user')." as a ";
	
	$sql .= " WHERE t. user_id = '$user_id' and t.admin_id = a.user_id ORDER BY add_time DESC";
	...
	while ($rows = $GLOBALS['db']->fetchRow($res))
	{
		...
		$msg[$rows['id']]['msg_content'] = nl2br(htmlspecialchars($rows['content']));
		$msg[$rows['id']]['msg_time']    = date("Y-m-d H:i:s",$rows['add_time']);	
		$msg[$rows['id']]['admin_name']    =$rows['admin_name'];	
	}
	return $msg;
}
