...
//     }
    ...
    $cat_id = get_article_cat_id($article_id);
    
    $top_id = get_article_cat_parent_id($cat_id);
...
else if($cat_id==14||$top_id==14||$cat_id==3||$top_id==3)
{
	if($top_id == 14)
	{
	...
	$smarty->assign('artciles_list',    get_cat_articles($top_id, 1, 30 ,$keywords));
...
function get_article_cat_id($article_id)
{
	$sql = "SELECT cat_id ".
			"FROM " .$GLOBALS['ecs']->table('article'). " AS a ".
...
function get_article_cat_parent_id($cat_id)
{
