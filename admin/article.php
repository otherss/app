...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
	...
	if ($exc->edit("sort_order = '$sort_order'", $goods_id))
	{
		clear_cache_files();
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
            ...
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '1', ".
...
if ($_REQUEST['act'] =='update')
{
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]',  author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]'", $_POST['id']))
    {
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'c.article_id' : trim($_REQUEST['sort_by']);
        ...
        /* 文章总数 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('article'). ' AS c '.
