...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $is_alone_sale = isset($_POST['is_alone_sale']) ? 1 : 1;
    ...
    $sql = "  select *  FROM " . $GLOBALS['ecs']->table('storage');
    ...
    if($_REQUEST['act'] == 'insert')
    {
	    ...
	    foreach ($res as $k => $v)
	    {
	    	$ss = " insert into ". $GLOBALS['ecs']->table('storage_goods')." (goods_id,storage_id, number) values ( ".$goods_id.",".$v['id'].",0 )";
	    	$GLOBALS['db']->query($ss);
	    }
    }
    /* 记录日志 */
