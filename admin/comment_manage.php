...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('comment_list.htm'), '',
        array('filter' => $list['filter'], 'page_count' => $list['page_count'],'select'=>storage_goods_list()));
}
...
function get_comment_list()
{
    ...
    $filter['cat_id']           = $_REQUEST['cat_id']<0? 0 : intval($_REQUEST['cat_id']);
    $filter['goods_id']           = $_REQUEST['goods_id']<0 ? 0 : intval($_REQUEST['goods_id']);
    ...
    //$filter['good']     = empty($_REQUEST['keywords']) ? 0 : trim($_REQUEST['keywords']);
    
    $filter['sort_by']      = empty($_REQUEST['sort_by']) ? 'add_time' : trim($_REQUEST['sort_by']);
    $filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

    $where = $filter['keywords']!=""? " AND content LIKE '%" . mysql_like_quote($filter['keywords']) . "%' " : '';
	
    $where .= $filter['goods_id']>0 ? " AND p1.goods_id =  ". $filter['goods_id']: '';
    
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comment'). " as p1 WHERE parent_id = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT p1.* , p2.user_name as name,p3.goods_name FROM " .$GLOBALS['ecs']->table('comment'). " as p1".
    		" left join " .$GLOBALS['ecs']->table('users'). " as p2 on p1.user_id = p2.user_id".
    		" left join " .$GLOBALS['ecs']->table('goods'). " as p3 on p1.goods_id = p3.goods_id".
    		" WHERE p1.parent_id = 0 $where " .
