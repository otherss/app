...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        //库存获取 - jack
        $row['storage'] = get_goods_storage($row['goods_id'],$order[order_id]);
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    else
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
        	$value['goods_number'] = get_goods_storage($value['goods_id'], $order_id);
...
//                                     WHERE goods_id = " . $value['goods_id'];
                ...
                $storage_id = get_storage_id($value['goods_id'],$order_id); 
                
                $minus_stock_sql = "UPDATE " . $GLOBALS['ecs']->table('storage_goods') . "
                      	              SET number = number - " . $value['sums'] . "
                          	         WHERE goods_id = " . $value['goods_id'] ." and storage_id = ".$storage_id;
    ...
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
              ...
              $goods_number_all = get_goods_storage($_POST['goods_id'][$key], $order_id);
...
//                 $num = $GLOBALS['db']->GetOne($sql);
                ...
                $num = get_goods_storage($value['goods_id'], $order_id);
...
function get_goods_storage($goods_id,$order_id)
{
...
// 	$sql = "select id from ". $GLOBALS['ecs']->table('storage') ." as p1  "
	
	$sql = " select number from ". $GLOBALS['ecs']->table('storage_goods') ." as p1 where storage_id =  ".$storage_id." and goods_id = ".$goods_id;
// 			." left join   "
