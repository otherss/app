...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('share_comment_list.htm');
}
...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('share_comment_list.htm'), '',
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许评论显示 */
        $sql = "UPDATE " .$ecs->table('share_comment'). " SET status = 1 ,check_time = ".gmtime()." WHERE id = '$_REQUEST[id]'";
        $db->query($sql);
        ...
        ecs_header("Location: share_comment_manage.php?act=reply&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止评论显示 */
        $sql = "UPDATE " .$ecs->table('share_comment'). " SET status = 2 ,check_time = ".gmtime()." WHERE id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$ecs->table('share_comment'). " WHERE id = '$id'";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$ecs->table('share_comment'). " WHERE pid = '$id'");
    }

    admin_log('', 'remove', 'ads');

    $url = 'share_comment_manage.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
                $db->query("DELETE FROM " . $ecs->table('share_comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'id'));
                $db->query("DELETE FROM " . $ecs->table('share_comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'pid'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $ecs->table('share_comment') . " SET status = 1 ,check_time = ".gmtime()." WHERE " . db_create_in($_POST['checkboxes'], 'id'));
               break;
           case 'reset' :
               $db->query("UPDATE " . $ecs->table('share_comment') . " SET status = 0 ,check_time = ".gmtime()." WHERE " . db_create_in($_POST['checkboxes'], 'id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $ecs->table('share_comment') . " SET status = 2 ,check_time = ".gmtime()." WHERE " . db_create_in($_POST['checkboxes'], 'id'));
               break;
        ...
        $link[] = array('text' => '返回晒单评论列表', 'href' => 'share_comment_manage.php?act=list');
 ...
 * @return  array
 */
function get_share_comment_list()
{
    /* 查询条件 */
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('share_comment'). " WHERE pid = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('share_comment'). " WHERE pid = 0 $where " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['check_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['check_time']);
