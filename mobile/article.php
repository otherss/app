...
if ($act == 'detail')
{
	...
	if ($a_id > 0)
	{
		$article_row = $db->getRow('SELECT title, add_time,content FROM ' . $ecs->table('article') . '  WHERE article_id = ' . $a_id . ' AND cat_id > 0 AND is_open = 1');
		if (!empty($article_row))
		{
			...
			$article_row['content'] = str_replace('src="','src="http://'.$_SERVER['HTTP_HOST'],$article_row['content']);			$article_row['content'] = htmlspecialchars_decode(encode_output($article_row['content']));
