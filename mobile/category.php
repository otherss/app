...
$c_id = !empty($_GET['c_id']) ? intval($_GET['c_id']) : 1;
	...
	if ($num > 0)
	{
		...
		foreach ($cat_goods['goods'] as $goods_data)
		{
			if (($i > ($page_num * ($page - 1 ))) && ($i <= ($page_num * $page)))
			{
				...
				//$wml_data .= "<a href='goods.php?id={$goods_data['id']}'>".encode_output($goods_data['name'])."</a>[".encode_output($price)."]<br/>";				$month_nums = ec_buysum($goods_data['id']);
				$data[] = array('i' => $i ,'month_nums'=>$month_nums ,'market_price' => $goods_data['market_price'] , 'price' => encode_output($price) , 'id' => $goods_data['id'] , 'name' => encode_output($goods_data['name']), 'thumb' => $goods_data['thumb'], 'goods_img' => $goods_data['goods_img']);//16:41 2013-07-16				
			}
			$i++;
		}
