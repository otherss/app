...
// 	);
	$result['cart_amount_desc'] = '购物金额'.$cart_goods['total']['goods_price'];
		$result['market_amount_desc'] = '节省了 ' .$cart_goods['total']['saving'].'（'.$cart_goods['total']['save_rate'].'）';
...
//         var_dump($consignee_list);
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'email'         => empty($_POST['email'])      ? 'weisai@qq.com' :   compile_str($_POST['email']),
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $consignee = formated_consignee($consignee);
//     var_dump($consignee);
    ...
    foreach ($shipping_list AS $key => $val)
    {
        ...
        if($total['goods_num'] >1 and $val['shipping_code']=="city_express")
        {
        	$shipping_fee = 0;
        }
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        $smarty->assign('order_max_integral', flow_available_points());  // 可用积风
        $smarty->assign('your_integral',      $user_info['pay_points']); // 用户积风
    }
...
else
{
    ...
    $smarty->assign('shopping_money', '购物金额<font class="f4" style="font-size:16px;" >'.$cart_goods['total']['goods_price'].'</font>');
    $smarty->assign('market_price_desc', '节省了 <font class="f4" style="font-size:16px;" >' .$cart_goods['total']['saving'].'（'.$cart_goods['total']['save_rate'].'）</font>');
